  ...
  return {
    ...
    history: pings.length
      ? pings.slice(0, 8).map((ping) => [
          ...
          `Location update from ${ping.device || "phone"}`
        ])
...
function renderMembers() {
  ...
  family.forEach((child) => {
    ...
    button.type = "button";
    button.innerHTML = `<strong>${child.name}</strong><span>${child.tracking ? "Live tracking" : child.paired ? "Paired, waiting for location" : "Pairing needed"}</span>`;
...
function renderHistory() {
  ...
  currentChild().history.forEach(([time, event]) => {
    ...
    row.innerHTML = `<div><strong>${event}</strong><span>Showing ${label}</span></div><span>${time}</span>`;
